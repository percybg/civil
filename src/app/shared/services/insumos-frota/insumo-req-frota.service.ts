  ...
  export class InsumosRequest {
    ...
    sendNewInsumo(form,metod,id = null){
      ...
      if(metod ==='POST'){
        ...
        req = this.http.post(`${this.sieconwebwebapi}/frotas/TermoResponsabilidade/inserirItemTermoResponsabilidade`,params);
      }else{
        params.itemId =id;
        req = this.http.put(`${this.sieconwebwebapi}/frotas/TermoResponsabilidade/inserirItemTermoResponsabilidade`,params);
      }
      return new Observable((observer) => {
        req.pipe(
          tap((response: any) => {
          }),
          switchMap((res: any) => this.requestService.getVersion(params.requisicaoId))
        ).subscribe(
          async (res) => {
            observer.next(res);
          },
